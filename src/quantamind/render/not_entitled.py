...
HEADER = "### QuantaMind"
SITE = "https://quantamind.co"
"""Only the fallback when a caller passes nothing. **The real value is `Settings.web_app_url`**,
because a staging deployment telling a customer to buy on the production site, or an on-prem one
pointing at a site its users cannot reach, is a dead end printed on somebody's pull request."""
NOTHING_READ = (
...
def _link(site: str, path: str) -> str:
    """A markdown link whose TEXT matches where it goes. The label was written out as
    `quantamind.co/account` beside a configurable URL, so a staging deployment produced a link that
    said one host and led to another — which reads as a phishing attempt, not a helpful one."""
    return f"[{site.split('://')[-1].rstrip('/')}/{path}]({site.rstrip('/')}/{path})"


def _why(admission: Admission, site: str) -> str:
    ...
    if reason == "private_needs_plan":
        return (
            ...
            f"tier covers public repositories. Plans start with a 14-day trial — "
            f"[see the plans]({site.rstrip('/')}/pricing)."
        )
    if reason == "seat_full":
        return (
            ...
            f"admin can add a seat or free one at {_link(site, 'account')}."
        )
...
def refusal(admission: Admission, site: str = SITE) -> str:
    ...
    **AN EMPTY REASON IS REFUSED.** A refusal that cannot say why is the silence this module
    ...
    why = _why(admission, site)
    return f"{HEADER}\n\n**This change was not reviewed.** {why}\n\n{NOTHING_READ}"


def seat_footer(admission: Admission, site: str = SITE) -> str:
    ...
    return (
        ...
        f"not run. An admin can add a seat at {_link(site, 'account')}._"
    )
